...
string	get_adapted_method_name(const ENTRY<MethodDef> &method) {
	if (method.name.begins("put_") || method.name.begins("get_") || method.name.begins("add_"))
		return fix_method_name(method.name.substr(4));

	if (method.name.begins("remove_"))
		return fix_method_name(method.name.substr(7));
...
template<typename C, typename L, typename R> struct comma_list_s {
	...
	void	operator()(string_accum &sa) const {
		int	j = from;
		for (auto &i : c)
			lambda(sa << onlyif(j++, ", "), i);
	};

	comma_list_s(const C &_c, L&& _lambda, int _from = 0) : c(_c), lambda(_lambda), from(_from) {}
};
...
struct Context {
	...
	hash_set2<const ENTRY<TypeDef>*>	reffed;
	hash_set2<const char *>				needed0;	// just forward defs (and enums) needed
	hash_set2<const char *>				needed;		// full include needed
	...
	const_memory_block get_attribute(Token tok, const char *type) const {
		for (auto &i : attributes.lookup(tok).or_default()) {
			if (i.Is(md, type))
				return i.Data();
		}
		return empty;
		...
		switch (tok.type()) {
			case TypeRef:
				if (auto *type = md.GetEntry<TypeRef>(tok.index()))
					return get_category(ref2def.lookup(type).or_default());
				break;
			case TypeDef:
	...
	const ENTRY<TypeDef> *GetTypeDef(Token tok) const {
		switch (tok.type()) {
			...
			case TypeRef:
				return ref2def.lookup(md.GetEntry<TypeRef>(tok.index())).or_default();

			case TypeSpec:
				ReadTypeToken(byte_reader(md.GetEntry<TypeSpec>(tok.index())->signature), tok);
	...
	const ENTRY<TypeDef> *GetTypeDef(const char *name) const {
		return name2def.lookup(namespce_name(name)).or_default();
	}
...
template<typename T> struct clr_graph : graph_base<clr_node<T> > {
	typedef clr_node<T>		N;

	hash_map<T, N*>	node_map;

	N *add_node(T t) {
		N*	&p = node_map[t];
		if (!p)
			p = graph_base<N>::add_node(new node_t(t));
		return p;
	}

	void add_edge(N *n, T t) {
		if (t && t != n->t) {
	...
	void add_edge (Context &ctx, N *n, byte_reader &r, int flags);
	void add_edges(Context &ctx, N *n, byte_reader &r, int flags);
	void add_edges(Context &ctx, N *n, const ENTRY<TypeDef> *def, int flags);
...
template<typename T> void clr_graph<T>::add_edge(Context &ctx, clr_node<T> *n, byte_reader &r, int flags) {
	...
	for (;;) {
		switch (t = ELEMENT_TYPE(r.getc())) {
			...
			case ELEMENT_TYPE_CLASS: {
				TypeDefOrRef	x	= CompressedUInt(r);
			...
			case ELEMENT_TYPE_GENERICINST: {
				...
				TypeDefOrRef	tmpl	= CompressedUInt(r);
...
template<typename T> void clr_graph<T>::add_edges(Context &ctx, clr_node<T> *n, const ENTRY<TypeDef> *def, int flags) {
	...
	for (auto &i : ctx.md.GetEntries(def, def->fields))
		add_edges(ctx, n, byte_reader(i.signature), flags);
...
//Kahn's algorithm
template<typename T> dynamic_array<T> clr_graph<T>::get_order() {
	dynamic_array<T*>		L;	// Empty list that will contain the sorted elements
	dynamic_array<node_t*>	S;	// Set of all nodes with no incoming edge
	for (auto &i : g.nodes) {
		if (i->outgoing.empty())
			S.push_back(i);
	}
	while (!S.empty()) {
		//remove a TypeNode n from S
		auto	n = S.pop_back_value();
		//add n to tail of L
		L.push_back(n);
		//for each TypeNode m with an edge e from n to m do
...
//			if m has no other incoming edges then
	...
	for (auto &i : g.nodes) {
		if (!i->outgoing.empty()) {
			ISO_TRACEF("Circular dependency: ") << i->def->namespce << '.' << i->def->name << '\n';
			for (auto &j : i->outgoing)
				ISO_TRACEF("\t") << j->def->namespce << '.' << j->def->name << '\n';
		}
	}
}
#endif
	...
	switch (tok.type()) {
		...
		case TypeRef:
			if (auto *ref = ctx.md.GetEntry<TypeRef>(tok.index())) {
				if (auto def = ctx.ref2def.lookup(ref).or_default()) {
					if (!(flags & 2) || (flags & 1)) {
						add_edge(n, def);
						if (flags & 1)
							ctx.needed.insert(def->namespce);
		...
		case TypeSpec:
			add_edge(ctx, n, byte_reader(ctx.md.GetEntry<TypeSpec>(tok.index())->signature), flags);
			break;
	}
}
...
struct AttributeElement {
	...
	template<typename R> bool read(ELEMENT_TYPE sig, R &data) {
		...
		switch (sig) {
			...
			case ELEMENT_TYPE_BOOLEAN:	 val = data.get<bool8>(); break;
			case ELEMENT_TYPE_CHAR:		 val = data.get<wchar_t>(); break;
			case ELEMENT_TYPE_I1:		 val = data.get<int8>(); break;
			case ELEMENT_TYPE_U1:		 val = data.get<uint8>(); break;
			case ELEMENT_TYPE_I2:		 val = data.get<int16>(); break;
			case ELEMENT_TYPE_U2:		 val = data.get<uint16>(); break;
			case ELEMENT_TYPE_I4:		 val = data.get<int32>(); break;
			case ELEMENT_TYPE_U4:		 val = data.get<uint32>(); break;
			case ELEMENT_TYPE_I8:		 val = data.get<int64>(); break;
			case ELEMENT_TYPE_U8:		 val = data.get<uint64>(); break;
			case ELEMENT_TYPE_R4:		 val = data.get<float>(); break;
			case ELEMENT_TYPE_R8:		 val = data.get<double>(); break;
			case ELEMENT_TYPE_I:		 val = data.get<int>(); break;
			case ELEMENT_TYPE_U:		 val = data.get<unsigned>(); break;
			...
			case ELEMENT_TYPE_VALUETYPE: {
				...
				switch (x.type()) {
					case TypeRef: {
						...
						def = ctx.ref2def.lookup(ref).or_default();
						break;
					}
					case TypeDef:
...
struct Methods {
	...
	void add(const ENTRY<TypeDef> *type, const ENTRY<MethodDef> &method) {
		...
		if (method.flags & ENTRY<MethodDef>::SpecialName) {
			auto	&array = methods[get_adapted_method_name(method)];
			for (auto &i : array) {
				if (i.type == type)
					return;
			}
...
struct HeaderWriter : Context {
	...
	HeaderWriter(const METADATA &_md, const char *_raw_prefix = "_") : Context(_md), current_ns(""), depth(0), lines(0), properties(true), events(true), indent_ns(false), try_block(true), raw_prefix(_raw_prefix)	{}

	void get_all_interfaces(hash_set2<Token> &found, const ENTRY<TypeDef> *def) {
		if (def) {
	...
	string	signature_type(const ENTRY<TypeDef> *def, byte_reader &r, FLAGS flags, const tmpl_params_t &tmpl_params) {
		string	s;
		write_signature_type(string_builder(s), def, 0, r, flags, tmpl_params);
		return s;
	}
	...
	if (f.flags & f.Literal) {
		...
		for (auto &j : md.GetTable<Constant>()) {
			if (j.parent == itok) {
				...
				sa << " = " << read_bytes<int64>(j.value, j.value.length32()) << ";\n";
				found = true;
				break;
			}
		}
...
string_accum &HeaderWriter::write_method_params(string_accum &sa, const ENTRY<TypeDef> *def, const tmpl_params_t &tmpl_params, const dynamic_array<param> &params, FLAGS flags) {
	int	n = params.size32();
	for (int i = 1; i < n; i++)
		write_signature_type(sa << onlyif(i > 1, ", "), def, params[i].name, byte_reader(params[i].sig), flags | USE_PARAM, tmpl_params);
	if (params[0].element != ELEMENT_TYPE_VOID)
		write_signature_type(sa << onlyif(n > 1, ", "), def, cstr("*") + params[0].name, byte_reader(params[0].sig), flags | POINTER, tmpl_params);
	return sa;
}
...
string_accum &HeaderWriter::write_method_params(string_accum &sa, const ENTRY<TypeDef> *def, const tmpl_params_t &tmpl_params, const dynamic_array<param> &params, param *ret_param, FLAGS flags) {
	...
	for (auto &i : params) {
		if (&i != ret_param && i.element != ELEMENT_TYPE_VOID)
			write_signature_type(sa << onlyif(j++, ", "), def, i.name, byte_reader(i.sig), ADAPTER | USE_PARAM, tmpl_params);
	}
	return sa;
}
...
string_accum& HeaderWriter::write_method(string_accum &sa, const ENTRY<TypeDef> *def, const ENTRY<MethodDef> &method, const tmpl_params_t &tmpl_params, bool abstract) {
	...
	if (method.name == ".dtor") {
		...
		if (auto overload = get_attribute(&method, "OverloadAttribute"))
			sa << SerString(byte_reader(overload));
		else
			sa << method.name;
	}
...
string_accum &HeaderWriter::write_adapted_method_body(string_accum &sa, const char *getter, const ENTRY<TypeDef> *def, const ENTRY<MethodDef> &method, const tmpl_params_t &tmpl_params, const dynamic_array<param> &params, param *ret_param, bool return_this) {
	...
	if (ret_param->element != ELEMENT_TYPE_VOID) {
		write_signature_type(sa, def, 0, byte_reader(ret_param->sig), ADAPTER | USE_TEMP, tmpl_params);
		sa << ' ' << ret_param->name << "; ";
	}
...
string_accum& HeaderWriter::write_overloaded_method(string_accum &sa, const ENTRY<TypeDef> *def, const ENTRY<TypeDef> *def2, const ENTRY<MethodDef> &method, const tmpl_params_t &tmpl_params) {
	...
	write_signature_type(sa, def, 0, byte_reader(ret_param->sig), ADAPTER | USE_RETURN | QUALIFY, tmpl_params);
...
param *HeaderWriter::get_return(dynamic_array<param> &params) {
	...
	if (ret->element == ELEMENT_TYPE_VOID) {
		for (auto &i : make_reversed(params)) {
			if (i.element & ELEMENT_TYPE_REF) {
...
bool HeaderWriter::used_name(const ENTRY<TypeDef> *def, const char *namespce, const char *name) {
	...
	for (auto &i : md.GetEntries(def, def->methods)) {
		if ((i.name.begins("put_") || i.name.begins("get_")) ? (i.name.substr(4) == name) : (i.name == name))
			return true;
...
string HeaderWriter::full_name(const ENTRY<TypeDef> *def, const Token &tok, const tmpl_params_t &tmpl_params, FLAGS flags) {
	switch (tok.type()) {
		...
		case TypeSpec:
			return signature_type(def, byte_reader(md.GetEntry<TypeSpec>(tok.index())->signature), flags, tmpl_params);
		default:
			return 0;
	}
}
...
ELEMENT_TYPE HeaderWriter::write_signature_type(string_accum &sa, const ENTRY<TypeDef> *def, const char *name, byte_reader &r, FLAGS flags, const tmpl_params_t &tmpl_params) {
	...
	for (;;) {
		switch (t = ELEMENT_TYPE(r.getc())) {
			...
			case ELEMENT_TYPE_VALUETYPE: {
				TypeDefOrRef	type	= CompressedUInt(r);
...
void HeaderWriter::write_property(string_accum &sa, const ENTRY<TypeDef> *def, const SemanticFunctions &sf, const tmpl_params_t &tmpl_params, const char *getter) {
	if (sf.Getter) {
		...
		param	*ret_param	= get_return(params);
		string	ret_type	= signature_type(def, byte_reader(ret_param->sig), ADAPTER | USE_RETURN, tmpl_params);
	...
	if (sf.AddOn) {
		...
		param	*ret_param	= get_return(params);
		write_signature_type(newline(sa), def, 0, byte_reader(ret_param->sig), ADAPTER | USE_RETURN, tmpl_params);
...
void HeaderWriter::write_statics(string_accum &sa, const ENTRY<TypeDef> *def, const dynamic_array<const ENTRY<TypeDef>*> &exclusives, const dynamic_array<string> &activators, const ENTRY<TypeDef> *composer) {
	...
	for (auto &method : md.GetEntries(def, def->methods)) {
		...
		if (const ENTRY<TypeDef> *exclusive = find_exclusive(md, exclusives, pmethod)) {
			...
			auto	params		= get_params(md, method);
			param	*ret_param	= get_return(params);
			write_signature_type(newline(sa) << "static ", def, 0, byte_reader(ret_param->sig), ADAPTER | USE_RETURN, tmpl_params);
	...
	for (auto &i : activators) {
		if (i == ".") {
			...
			for (auto &method : md.GetEntries(activator, activator->methods)) {
				...
				for (int i = 1; i < n; i++)
					write_signature_type(sa << onlyif(i > 1, ", "), def, params[i].name, byte_reader(params[i].sig), ADAPTER | USE_PARAM, 0);
	...
	if (composer) {
		for (auto &method : md.GetEntries(composer, composer->methods)) {
			...
			for (int i = 1, j = 0; i < n; i++)
				write_signature_type(sa << onlyif(j++, ", "), def, params[i].name, byte_reader(params[i].sig), ADAPTER | USE_PARAM, 0);
...
//			ISO_ASSERT(method.name!="First");
			...
			if (properties) {
				if (method.name.begins("put_")) {
					...
					sa << method.name.substr(4) << "+=";
				} else if (method.name.begins("remove_")) {
					sa << method.name.substr(7) << "-=";
				} else {
					close = ")";
					sa << method.name << '(';
				}
...
void MakeMDH(const METADATA &md, string_accum &sa0, string_accum &sa, const dynamic_array<const char*> &namespaces) {
	...
	for (auto &ns : all_ns) {
		for (part_iterator<'.'> i = ns; *i; ++i)
			hw.name2def[namespce_name(i.full())];	// create dummy entry for namespace
	}
	...
	for (auto &i : md.GetTable<InterfaceImpl>()) {
		...
		if (g.node_map.check(def)) {
			...
			auto &interfaces = hw.interfaces[def];
...
//		g_ns.add_node(type.namespce);
	...
	for (auto def : ordered_defs) {
		...
		if (auto comp = composable.lookup(def).or_default()) {
			sa << ", composer_type<" << hw.full_name(def, comp) << '>';
		} else {
			auto	&acts		= activators[def];
			if (acts && (acts.size() != 1 || acts[0] != "."))
		...
		switch (cat) {
			case hw.CAT_INTERFACE: {
				...
				if (auto e2 = hw.get_attribute(def, "ExclusiveToAttribute")) {
					const ENTRY<TypeDef> *parent = hw.name2def[namespce_name(SerString(byte_reader(e2)))];
			...
			case hw.CAT_CLASS: {
				...
				auto	composer	= composable.lookup(def).or_default();
				auto	&acts		= activators[def];
				bool	has_acts	= composer || (acts && (acts.size() != 1 || acts[0] != "."));
