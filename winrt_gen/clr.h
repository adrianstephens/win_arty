...
struct StreamHdr {
	...
	char		Name[1];	// Name of the stream as null-terminated variable length array of ASCII characters, padded to the next 4-byte boundary with \0 characters. The name is limited to 32 characters.
	StreamHdr	*next()	const { return (StreamHdr*)(Name + align(strlen(Name) + 1, 4)); }
};

struct METADATA_ROOT : littleendian {
	enum {SIGNATURE = 'BSJB'};
	uint32		Signature;
	uint16		MajorVersion;
	uint16		MinorVersion;
...
struct String : string_base<const char*> {
	template<typename R> String(R &r)	: string_base<const char*>(r.GetString())	{}
};
struct Blob : const_memory_block {
	Blob()		{}
	template<typename R> Blob(R &r)		: const_memory_block(r.GetBlob())	{}
};
...
struct Code {
	...
	template<typename R> Code(R &r)		: start(r)	{}
};
template<TABLETYPE I> struct Indexed : WRAP<uint32> {
	Indexed(uint32 _i)	: WRAP<uint32>(_i)	{}
	template<typename R> Indexed(R &r)	: WRAP<uint32>(r.GetIndex(I))	{}
};
...
template<typename T>	struct CodedIndex {
	static uint8 trans[];
	uint32		i;
	TABLETYPE	type()		const	{ return TABLETYPE(trans[i & ((1 << T::B) - 1)]); }
	uint32		index()		const	{ return i >> T::B; }
	operator	safe_bool::type()	const	{ return safe_bool::test(i);}
...
struct Token {
	...
	inline_only	operator safe_bool::type()	const { return safe_bool::test(index());}
	Token()						{}
...
template<> struct ENTRY<Constant> {
	...
	HasConstant			parent;
	Blob				value;
	template<typename R> explicit ENTRY(R &r) : type(r.get<compact<ELEMENT_TYPE,16> >()), parent(r), value(r) {}
};
...
struct METADATA_READER : byte_reader {
	METADATA	*c;
	uint8		heapsizes;

	METADATA_READER(const void *_p, METADATA *_c, uint8 _heapsizes) : byte_reader(_p), c(_c), heapsizes(_heapsizes) {}
	...
	template<TABLETYPE I>	void ReadTable() {
		int		n	= c->tables[I].n;
		ENTRY<I>	*p	= (ENTRY<I>*)(c->tables[I].p = malloc(sizeof(ENTRY<I>) * n));
		for (int i = 0; i < n; i++)
