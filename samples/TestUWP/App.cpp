...
void test() {
	...
	feed->Progress([](auto &&sender, const RetrievalProgress &p) {
	...
	for (auto i : wait(client->RetrieveFeedAsync(uri))->Items()) {
...
int __stdcall wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
	auto	install = Windows::ApplicationModel::Package::Current->InstalledLocation();
	auto	dir		= install->Path();


	auto	v = (Composition::Visual*)0;
...
#else
	Windows::ApplicationModel::Core::CoreApplication::Run(new BlocksApp);
#endif
}
