...
#if 0//def USE_FILENAME_WINDOWS

filename::drive_t filename::drive() const {
	drive_t	t;
	_splitpath(*this, t, NULL, NULL, NULL);
	return t;
}
...
filename filename::name_ext() const {
	...
	ext_t		e;
	filename	t;
	_splitpath(*this, NULL, NULL, n, e);
	_makepath(t, NULL, NULL, n, e);
	return t;
}
...
filename filename::dir_name_ext() const {
	...
	filename	n;
	ext_t		e;
	_splitpath(*this, NULL, d, n, e);
	_makepath(d, NULL, d, n, e);
	return d;
}

filename filename::name() const {
	filename	t;
	_splitpath(*this, NULL, NULL, t, NULL);
	return t;
}

filename::ext_t filename::ext() const {
	ext_t	t;
	_splitpath(*this, NULL, NULL, NULL, t);
	return t;
}

#else// USE_FILENAME_UNIX
...
filename filename::relative(const char *f) const {
	...
	if (!fn.is_relative())
		return fn;
	return filename(dir()).add_dir(f);
}

filename filename::relative_to(const char *f) const {
	const char	*a	= *this;
	const char	*dir = 0;
	while (compare(ichar(*a), *f) == 0) {
		if (IsDirSep(*a))
			dir = a;
		a++;
		f++;
...
bool create_dir(const char *f) {
	return !f || !f[0]
		...
		|| (create_dir(filename(f).rem_dir()) && CreateDirectoryA(f, NULL) == 0);
//		|| _mkdir(f) == 0 || errno == EEXIST
//		|| (create_dir(filename(f).rem_dir()) && _mkdir(f) == 0);
}
...
HMODULE load_library(const char *name, const char *env, const char *dir) {
	filename	fn	= filename(getenv(env)).add_dir(dir).add_dir(name);
	...
	string	path	= string(getenv("path")) + ";" + (env ? (const char*)filename(getenv(env)).add_dir(dir) : dir);
