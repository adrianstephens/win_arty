...
auto operator co_await(const ptr<Windows::Foundation::IAsyncAction> &p) {
	struct awaiter {
...
struct apartment_context {
	...
	apartment_context() {
		hrcheck(CoGetObjectContext(__uuidof(context), (void**)&context));
	}
	~apartment_context() {
		context->Release();
	}
	...
	void await_suspend(std::experimental::coroutine_handle<> handle) const {
		...
		hrcheck(context->ContextCallback(callback, &data, IID_ICallbackWithNoReentrancyToApplicationSTA, 5, nullptr));
	}
};
