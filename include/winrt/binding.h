...
namespace XamlTypeInfo {
	...
	public:
		XamlUserType(Provider* _provider, hstring_ref fullName, pptr<IXamlType> baseType) :
			provider(_provider),
			IsArray(false), IsMarkupExtension(false), IsBindable(false),
			FullName(fullName),
			BaseType(baseType),
			IsEnum(false), IsReturnTypeStub(false), IsLocalType(false)
		{}
	...
	class XamlMember : public runtime<XamlMember, IXamlMember> {
		Provider				*provider;
		const member			*info;
		hstring					target;
	public:
		XamlMember(Provider* _provider, const member *_info) : provider(_provider), info(_info), IsAttachable(false), IsDependencyProperty(false) {}
	...
	class XamlSystemBaseType : public runtime<XamlSystemBaseType, IXamlType, IWeakReferenceSource> {
		ptr<weak_reference_imp>	weak;
		STDMETHODIMP GetWeakReference(IWeakReference **weakReference) {
			*weakReference	= weak.make(query<IInspectable>(this));
			return S_OK;
	...
	public:
		XamlSystemBaseType(hstring_ref name) : FullName(name) {}

		hstring				FullName;
		Interop::TypeName	UnderlyingType()						{ return {FullName, Interop::TypeKind::Primitive}; }
		IXamlType*			BaseType()								{ throw NotImplementedException(); }
		IXamlMember*		ContentProperty()						{ throw NotImplementedException(); }
		bool				IsArray()								{ throw NotImplementedException(); }
		bool				IsCollection()							{ throw NotImplementedException(); }
		bool				IsConstructible()						{ throw NotImplementedException(); }
		bool				IsDictionary()							{ throw NotImplementedException(); }
		bool				IsMarkupExtension()						{ throw NotImplementedException(); }
		bool				IsBindable()							{ throw NotImplementedException(); }
		...
		IXamlMember*		GetMember(hstring_ref name)				{ throw NotImplementedException(); }
		...
		HRESULT				RunInitializer()						{ throw NotImplementedException(); }
	...
	public:
		ptr<IXamlType> GetXamlType(hstring_ref typeName) {
			...
			if (auto *info = type_id::find(typeName)) {
				XamlUserType	*type = new XamlUserType(this, typeName, GetXamlType(info->base));
			...
			for (auto &i : system_types) {
				if (typeName == str(i)) {
					XamlSystemBaseType	*type	= new XamlSystemBaseType(typeName);
					types[typeName]		=  type;
					return type;
				}
			}
		...
		ptr<IXamlType> GetXamlType(Interop::TypeName type) {
			auto	xaml	= GetXamlType(type.Name);
	...
	private:
		...
		IXamlMember* CreateXamlMember(hstring_ref longMemberName) {
			if (auto *info = GetMemberInfo(longMemberName))
				return new XamlMember(this, info);
			return nullptr;
		}
	...
	inline ptr<IXamlType>	XamlUserType::ItemType()		{ return provider->GetXamlType(itemTypeName); }
	inline ptr<IXamlType>	XamlUserType::KeyType()			{ return provider->GetXamlType(keyTypeName); }
	...
	inline ptr<IXamlType>	XamlMember::Type()				{ return provider->GetXamlType(info->type); }
	inline ptr<IXamlType>	XamlMember::TargetType()		{ return provider->GetXamlType(target); }
}
