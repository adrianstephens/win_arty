...
template<size_t N> constexpr GUID bytes_to_guid(const constexpr_array<uint8, N> &arr) {
	return {
		bytes_to_u4(arr[3], arr[2], arr[1], arr[0]),
		bytes_to_u2(arr[5], arr[4]),
		bytes_to_u2(arr[7], arr[6]),
		{arr[8], arr[9], arr[10], arr[11], arr[12], arr[13], arr[14], arr[15]}
	};
}

template<size_t N> constexpr GUID string_to_guid(constexpr_array<char, N> &a) {
	return GUID{
...
template<size_t N> constexpr GUID string_to_guid(const char (&a)[N]) {
	return string_to_guid(make_constexpr_string(a));
}

constexpr GUID operator "" _guid(const char *p, size_t len) {
	return string_to_guid(*(constexpr_array<char,36>*)(p));
}

constexpr constexpr_array<char, 38> guid_to_string(const GUID &id) {
	return "{"
...
template<size_t N> constexpr constexpr_array<uint8, N + 16> create_guid_gen_buffer(const GUID &guid, const constexpr_array<char, N> &str) {
	return to_byte_array(guid) + make_constexpr_array<uint8>(str);
}
...
template<typename S> constexpr GUID generate_interface_guid(const S &sig) {
	...
	return set_named_guid_fields(bytes_to_guid(SHA1::calculate(create_guid_gen_buffer(namespace_guid, sig))));
}
...
template<char... vv>		struct fix_constexpr_name2													: value_list<char, vv...> {};
...
template<char... vv>		struct fix_constexpr_name2<'i','s','o','_','w','i','n','r','t','.',	vv...>	: value_list<char, vv...> {};
...
template<char u0, char u1, char... vv>		struct fix_constexpr_name<value_list<char, u0, u1>,				value_list<char, vv...> >	: fix_constexpr_name2<vv..., u0, u1>		{};

//auto __cdecl iso_winrt::get_constexpr_name_s<class <namespace>::<id>>::f(void)
template<typename T> struct get_constexpr_name_s {
	constexpr static auto f() { return trim_back<10>(trim_front<45>(make_constexpr_string(__FUNCSIG__))); };
...
template<typename T> struct pptr<Platform::Array<T>> : szarray<T> {
	pptr()								{ this->p = 0; this->size = 0; }
	pptr(T *a, size_t n)				{ this->p = a; this->size = n; }
	template<size_t N> pptr(T (&a)[N])	{ this->p = a; this->size = N; }
};

template<typename T> struct ptr<Platform::Array<T>> : szarray<T> {
	...
	ptr()										{ this->p = 0; this->size = 0; }
	ptr(ptr &&b)								{ this->p = b.detach(&this->size); }
...
template<typename T> struct name { static constexpr auto value = make_constexpr_array<wchar_t>(get_constexpr_name<T>()); };// + wchar_t(0); };
...
#define const_sig(s)	static const auto sig() { static auto &sig{s}; return make_constexpr_string(sig); }
#define get_sig(S)		S::sig()
#endif
...
template<typename T> struct signature {
	...
	SIG_CONSTEXPR static auto get(basic_type*) {
		return make_constexpr_string(def_runtime<T>::sig);
	};
	template<typename U> SIG_CONSTEXPR static auto get(enum_type<U>*) {
		return "enum(" + name<T>::value + ";" + sigof<U>() + ")";
	};
	template<typename... TT> SIG_CONSTEXPR static auto get(value_type<TT...> *) {
		return "struct(" + make_constexpr_string<char>(name<T>::value) + ";" + sigof<types<TT...>>() + ")";
	}
	template<typename E, typename I0, typename... I> SIG_CONSTEXPR static auto get(class_type<E, I0, I...> *) {
		return "rc(" + make_constexpr_string<char>(name<T>::value) + ";" + guid_to_string(uuidof<I0>()) + ")";
	}
...
template<typename F, typename T> struct query_s<F, T, false> {
	static ptr<T> ref(F *f) {
		T *t;
		return f && SUCCEEDED(f->QueryInterface(uuidof<T>(), (void**)&t)) ? t : 0;
	}
};
...
template<typename T> class pptr_base {
	...
	bool	release;
public:
	pptr_base(T *p)					: t(p), release(false)			{}
	pptr_base(ptr<T> &&p)			: t(p.detach()), release(true)	{}
...
public:
	...
	ptr(T *p)				: B(p)					{}
	ptr(ptr &&p)			: B(p.detach())			{}
	ptr(const ptr &p)		: B(p.get_addref())		{}
	ptr(pptr<T> &p)			: B(p.get_addref())		{}
	ptr(pptr<T> &&p)		: B(p.get_addref())		{}
	ptr(const _ptr_base &p)	: B(query<T>(p.get0()))	{}
	ptr(_ptr_base &&p)		: B(query<T>(p.get0()))	{}
	template<typename A> ptr(const ptr<A> &p)	: B(query<T>(p.get()), true)	{}

	ptr&						operator=(ptr &&p)			{ swap(this->t, p.t); return *this; }
	ptr&						operator=(const ptr &p)		{ set(p.t); return *this; }
...
public:
	pptr(T *p)				: B(p)					{}
	pptr(const ptr<T> &p)	: B(p.get())			{}
	pptr(ptr<T> &&p)		: B(move(p))			{}
	template<typename A> pptr(A *a)				: B(query<T>(a))		{}
	template<typename A> pptr(const ptr<A> &a)	: B(query<T>(a.get()))	{}
};
...
template<> class pptr<IInspectable>;
...
public:
	...
	ptr(pptr<T> &&p);
	template<typename A> ptr(const ptr<A> &p)	: B(query<T>(p.get()), true)	{}
	template<typename A> ptr(ptr<A> &&p)		: B(query<T>(p.detach()))		{}
	template<typename A> ptr(const pptr<A> &p)	: B(query<T>((A*)p), true)		{}
	template<typename A> explicit ptr(A *p)		: B(query<T>(p), true)			{}
...
public:
	...
	pptr(ptr<T> &&p)		: B(move(p))			{}
	template<typename A>	pptr(const ptr<A> &a)	: B(query<T>(a.get()))	{}
	template<typename A>	explicit pptr(A *a)		: B(query<T>(a))		{}
...
inline ptr<IInspectable>::ptr(pptr<IInspectable> &p)	: B(p.get_addref())	{}
inline ptr<IInspectable>::ptr(pptr<IInspectable> &&p)	: B(p.get_addref())	{}
...
protected:
	...
	template<typename I> bool init(I s, I e) {
		...
		if (SUCCEEDED(WindowsPreallocateStringBuffer((UINT32)string_copy_len<wchar_t>(s, e), &buffer, &handle))) {
...
public:
	...
	hstring_base(HSTRING _h = nullptr)	: h(_h)		{}

	HSTRING*		operator&()	{ ISO_ASSERT(!h); return &h; }

	operator HSTRING()							const	{ return h; }
	const HSTRING*	operator&()					const	{ return &h; }
	...
	hstring			copy()									const;
	hstring			substr(int i)							const;
	hstring			substr(int i, int n)					const;
	...
	friend int		compare(const hstring_base &a, const hstring_base &b) {
		int r;
...
//	template<size_t N> pptr(const constexpr_array<wchar_t, N> &s)	{ CreateReference(s.begin(), N - 1); }//must be zero-terminated
	template<typename C, size_t N> pptr(const constexpr_array<C, N> &s)	{ init(s.begin(), s.begin() + N); }//CreateReference(s.begin(), N, N - 1); }//not zero-terminated
...
template<typename T> struct from_abi_s<T*>		{ typedef ptr<T> type; };
...
template<typename T> inline ptr<T>	from_abi(T *t)				{ return t; }
...
//	template<T (X::*p)()>				static auto get(X *x, R *r)		{ return to_abi(x->*p)()); }// causes compiler crash
	template<typename V> static auto	get2(V &&v)						{ return v; }
...
template<typename X, typename T> prop_getter<T (X::*)()>		get_prop_getter(T (X::*p)())		{ return prop_getter<T (X::*)()>(); }
#define get_prop(rr, p)	{ typedef X::type T; auto t = X::get(); auto r = rr; get_prop_getter(&T::p).get<&T::p>(t, r); }
...
#define put_prop(rr, p)	{ typedef X::type T; auto t = X::get(); auto r = rr; get_prop_putter(&T::p).put<&T::p>(t, r); }
